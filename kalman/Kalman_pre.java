...
public class Kalman_pre {
	...
	public static void main(String[] args) throws Exception{
		...
		//File temp = new File("D:\\ais_2012\\for liner\\temp2");
		//File temp = new File("D:\\ais_2012\\for liner\\for cov");//covの計算のため
		ENU enu = new ENU();
		SHIP S = new SHIP();
		...
		Matrix ProsessNoiseCov = new Matrix(
				new double[][] {{ 0.5540341524701068, 0.03958970771784662, 0.039589692819525964,0.039589698114432394},
		...
		Matrix MeasurementNoiseCov = new Matrix(
				new double[][]{{ 100.05074019856619,0.050740198730252886,0.050740197719993475,0.050740198905271476},
		...
		for(int i=0; i<filenum;i++){
			...
			try {
				...
				//forpainting
				//double forpaint[][] = new double[DataCount-1][4];//予測緯度、予測経度、緯度、経度の順
				...
				s.set(3, 0, dy);
				//
				Matrix c = new Matrix(4, 1); // corrected state [x, y, dx, dy]、事後推定値
				c.set(0, 0, 0);
				c.set(1, 0, 0);
				Matrix m = new Matrix(4, 1); // measurement [x]
				m.set(0, 0, 0);
				m.set(1, 0, 0);
				m.set(2, 0, dx);
				m.set(3, 0, dy);
				...
				for(int j = 0; j<DataCount -1801; j++){
					...
					m.set(2, 0, ((SHIP)ShipData[j+1]).speed * 1852/3600 *  Math.cos(Math.toRadians(((SHIP)ShipData[j+1]).course)));
					m.set(3, 0, ((SHIP)ShipData[j+1]).speed * 1852/3600 *  Math.sin(Math.toRadians(((SHIP)ShipData[j+1]).course)));
					c = kalman.Correct(m);
	...
	public static void SaveChartAsFile(String csvfilename, JFreeChart chart)throws IOException{
		...
		private static void writeCSV(double total_err ,int i ,double count){
			// TODO 自動生成されたメソッド・スタブ
			File errfile = new File("D:\\ais_2012\\for kalman\\total_err\\KalmanTotalErr.csv");
			//File errfile = new File("D:\\ais_2012\\for kalman\\total_err\\KalmanTotalErrTest.csv");
			try {
		...
		private static void writeVar(double total_var,int i, double count){
			File varfile = new File("D:\\ais_2012\\for kalman\\total_err\\Kalman_Variance.csv");
			try{
